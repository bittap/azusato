  ...
  class getLoginUser {
    ...
    public void givenNormalCase_result200() throws Exception {
      ...
      MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders
          ...
          .with(user(TestLogin.adminLoginUser())).with(csrf())).andDo(print())
      ...
      GetSessionUserServiceAPIResponse expect =
          GetSessionUserServiceAPIResponse.builder().id(Entity.createdVarChars[0])
    ...
    public void givenNoLogin_result400(Locale locale) throws Exception {
      MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders
          .get(TestConstant.MAKE_ABSOLUTE_URL + Api.COMMON_REQUSET + UserControllerAPI.COMMON_URL)
          .with(csrf()).locale(locale)).andDo(print()).andExpect(status().isBadRequest())
          .andReturn();
    ...
    public void givenNotExistData_result500(Locale locale) throws Exception {
      ...
      ErrorResponse expect = new ErrorResponse(AzusatoException.I0005,
    ...
    public void givenAlreadyCookie_result400(Locale locale) throws Exception {
      ...
      MvcResult mvcResult = mockMvc
          .perform(MockMvcRequestBuilders
              .post(TestConstant.MAKE_ABSOLUTE_URL + Api.COMMON_REQUSET
                  + UserControllerAPI.ADD_NONMEMBER_URL)
              .with(csrf()).locale(locale).contentType(HttpConstant.DEFAULT_CONTENT_TYPE_STRING)
      ...
      assertEquals(new ErrorResponse(AzusatoException.I0003,
          messageSource.getMessage(AzusatoException.I0003, null, locale)), result);
    }

    @ParameterizedTest
    @MethodSource("com.my.azusato.integration.api.controller.UserContollerAPITest#givenInvaildParameter_result400")
    public void givenInvaildParameter_result400(Locale locale, AddNonMemberUserAPIRequest req,
      ...
      MvcResult mvcResult = mockMvc
          .perform(MockMvcRequestBuilders
              .post(TestConstant.MAKE_ABSOLUTE_URL + Api.COMMON_REQUSET
                  + UserControllerAPI.ADD_NONMEMBER_URL)
              .with(csrf()).content(requestBody)
              .contentType(HttpConstant.DEFAULT_CONTENT_TYPE_STRING).locale(locale))
    ...
    return Stream.of(
        Arguments.of(TestConstant.LOCALE_JA,
            AddNonMemberUserAPIRequest.builder().name(RandomStringUtils.randomAlphabetic(11))
                .build(),
            "名前は最大10桁数まで入力可能です。"),
        Arguments.of(TestConstant.LOCALE_JA, AddNonMemberUserAPIRequest.builder().build(),
            "名前は必修項目です。"),
        Arguments.of(TestConstant.LOCALE_KO, AddNonMemberUserAPIRequest.builder().build(),
            "이름을 입력해주세요."),
        Arguments.of(
            TestConstant.LOCALE_KO, AddNonMemberUserAPIRequest.builder()
