...
public class CelebrationContrllerTest extends AbstractIntegration {
	...
	public class Write {
		...
		@ParameterizedTest
		@ValueSource(strings = { UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL, 
				UrlConstant.JAPANESE_CONTROLLER_REQUEST + UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL,
				UrlConstant.KOREAN_CONTROLLER_REQUEST + UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL })
		...
		public void givenNoSession_resultOk() throws Exception{
			MvcResult mvcResult = mockMvc.perform(
						MockMvcRequestBuilders.get(UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL)
						)
					.andDo(print())
	...
	public class Modify {
		...
		@ParameterizedTest
		@ValueSource(strings = { UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL + celebrationNoPath, 
				UrlConstant.JAPANESE_CONTROLLER_REQUEST + UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL + celebrationNoPath,
				UrlConstant.KOREAN_CONTROLLER_REQUEST + UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL + celebrationNoPath })
		public void givenUrl_resultNot404(String url) throws Exception {
			mockMvc.perform(MockMvcRequestBuilders.get(url)).andExpect(status().is(200));
		}
		...
		public void givenNoSession_resultOk() throws Exception{
			MvcResult mvcResult = mockMvc.perform(
						MockMvcRequestBuilders.get(UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL + celebrationNoPath)
						)
					.andDo(print())
					.andExpect(status().isOk()).andReturn();
			...
			CelebrationModifyResponse response = (CelebrationModifyResponse)mapsOfResult.get(ModelConstant.DATA_KEY);
			CelebrationModifyResponse expect = CelebrationModifyResponse.builder().celebrationNo(Long.valueOf(celebrationNo)).build();
		...
		public void givenBadRequest_result400(Locale locale,String url, String expectedMessage) throws Exception {
	...
	private static Stream<Arguments> modify_givenBadRequest_result400() {
		...
		return Stream.of(
				Arguments.of(TestConstant.LOCALE_JA,
						UrlConstant.CELEBRATION_CONTROLLER_REQUSET + CelebrationController.CELEBRATION_WRITE_URL + "/string",
						"不正な値が存在します。"),
