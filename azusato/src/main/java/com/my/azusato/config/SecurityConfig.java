...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    ...
    for (Locale locale : LocaleConstant.supportedLocales) {
      onlyAdminUrls.add(String.format("/%s%s%s", locale.toString(),
          UrlConstant.COMMON_ADMIN_CONTROLLER_REQUSET, "/**"));
    }
    ...
    http.authorizeRequests() //
        ...
        .and() //
        // フレーム要素を許可するかどうか。
        // sameOrigin : 同じサイトのフレーム要素は許可する。
        // => h2-consoleDBはフレームのため
        .headers().frameOptions().sameOrigin() //
        .and() //
        .csrf().ignoringAntMatchers("/h2-console/**") // h2-consoleはcsrfチェックを除外する。
        .and() //
        .formLogin() //
        ...
        .successHandler(new AuthenticationSuccessHandler() {
          ...
          /**
           * ログインに成功した場合。 「ユーザID保存」のチェックボックスがcheckedの場合は、「ユーザID保存」に対するクッキーに保持する。
          ...
          public void onAuthenticationSuccess(HttpServletRequest request,
            ...
            if (Objects.isNull(savedId)) {
              log.warn("ログインに成功したものの、パラメータに\"{}\"が含まれてないためユーザID保存の追加・削除が行われていなかったです。",
                  cookieProperty.getLoginSaveIdName());
            } else {
              // true
          ...
          public void onAuthenticationFailure(HttpServletRequest request,
            ...
            if (ex instanceof UsernameNotFoundException) {
              ...
              errorResponse
                  .setMessage(ms.getMessage(AzusatoException.I0009, null, request.getLocale()));
              statusCode = HttpStatus.BAD_REQUEST.value();
            } else if (ex instanceof BadCredentialsException) {
              errorResponse.setTitle(AzusatoException.I0010);
              errorResponse
                  .setMessage(ms.getMessage(AzusatoException.I0010, null, request.getLocale()));
              statusCode = HttpStatus.BAD_REQUEST.value();
            } else {
              ...
              errorResponse
                  .setMessage(ms.getMessage(AzusatoException.E0001, null, request.getLocale()));
        ...
        .accessDeniedHandler(new AccessDeniedHandler() {
          ...
          public void handle(HttpServletRequest request, HttpServletResponse response,
            ...
            if (ex instanceof InvalidCsrfTokenException) {
              ...
              errorResponse
                  .setMessage(ms.getMessage(AzusatoException.W0002, null, request.getLocale()));
  ...
  /**
   * クッキーを追加する。 {@link SecurityConfig#addOrdeleteSaveIdCookie(String, HttpServletResponse)}に処理を委員する。
   */
  ...
  /**
   * クッキーを削除する。 {@link SecurityConfig#addOrdeleteSaveIdCookie(String, HttpServletResponse)}に処理を委員する。
   */
