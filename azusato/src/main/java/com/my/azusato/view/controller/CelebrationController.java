...
public class CelebrationController {
  ...
  public String redirectListFromNotice(
      @PathVariable(name = "no", required = true) Long celebrationNo,
      @PathVariable(name = "replyNo", required = false) Long celebrationReplyNo) {
    log.debug("redirectListFromNotice controller");

    Integer currentPageNo = celeAPIService.getPage(celebrationNo);
    ...
    return LocaleInterceptor.getLocale(servletRequest) == Locale.KOREAN
        ? "redirect:" + UrlConstant.JAPANESE_CONTROLLER_REQUEST
            + UrlConstant.CELEBRATION_CONTROLLER_REQUSET + "?" + queryParameter
        : "redirect:" + UrlConstant.KOREAN_CONTROLLER_REQUEST
            + UrlConstant.CELEBRATION_CONTROLLER_REQUSET + "?" + queryParameter;
  }
  ...
  public ModelAndView modify(@PathVariable(name = "no", required = true) Long celebrationNo) {
    ...
    addHeaderModel(mav);
    // model response 設定
    mav.addObject(ModelConstant.DATA_KEY,
        CelebrationModifyResponse.builder().celebrationNo(celebrationNo).build());
